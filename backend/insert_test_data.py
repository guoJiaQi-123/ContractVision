...
def create_contracts(users):
    ...
    clients = [
        ...
        '京东集团', '美团点评', '小米科技', '网易公司',
        '百度在线', '滴滴出行', '美团外卖', '拼多多',
        '携程旅行', '京东商城', '阿里巴巴国际', '华为云服务',
        '腾讯云', '阿里云', '百度云', '腾讯游戏',
        '网易游戏', '京东金融', '蚂蚁金服', '腾讯支付',
        '百度金融', '美团金融', '京东支付', '支付宝'
    ]
    products = [
        '云服务', 'SaaS软件', '数据平台', '企业ERP',
        'CRM系统', 'OA办公', '电商平台', '数据分析',
        '云存储', '人工智能', '大数据平台', '物联网'
    ]
    ...
    for i in range(1, 31):
        ...
        contract, _ = Contract.objects.get_or_create(
            ...
            defaults={
                ...
                'contract_type': random.choice(['软件销售', '服务采购', '设备采购', '云订阅']),
                ...
                'renewal_owner': random.choice(users) if users else None,
            }
        )
...
def create_contract_related_data(contracts, users):
    ...
    for index, contract in enumerate(contracts, start=1):
        if not contract.payment_plans.exists():
            for phase_index, phase_name in enumerate(['预付款', '中期款', '尾款'], start=1):
                due_date = contract.start_date + timedelta(days=phase_index * 30) if contract.start_date else datetime.now().date()
                PaymentPlan.objects.get_or_create(
                    contract=contract,
                    phase=phase_name,
                    defaults={
                        'ratio': Decimal('30.00') if phase_index < 3 else Decimal('40.00'),
                        'amount': (contract.amount or Decimal('0')) * Decimal('0.3') if phase_index < 3 else (contract.amount or Decimal('0')) * Decimal('0.4'),
                        'actual_amount': Decimal('0.00'),
                        'due_date': due_date,
                        'status': random.choice(['pending', 'paid', 'overdue']),
                        'invoice_status': random.choice(['pending', 'issued', 'received']),
        ...
        if not contract.milestones.exists():
            ...
            for step_index, (node_type, name, weight) in enumerate(milestone_specs, start=1):
                planned_date = contract.start_date + timedelta(days=step_index * 25) if contract.start_date else datetime.now().date()
                ContractMilestone.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'status': 'completed' if step_index == 1 else random.choice(['not_started', 'in_progress', 'overdue']),
        ...
        if not contract.change_requests.exists():
            ContractChangeRequest.objects.get_or_create(
                ...
                defaults={
                    ...
                    'after_snapshot': {'payment_status': 'partial', 'description': f'{contract.description}\n已申请调整付款条款。'},
...
def create_governance_data(users, contracts):
    ...
    for contract in contracts[:8]:
        ...
        ApprovalRequest.objects.get_or_create(
            ...
            defaults={
                'request_payload': {'contract_id': contract.id, 'contract_no': contract.contract_no},
                'status': random.choice(['pending', 'approved', 'rejected']),
                'requested_by': random.choice(users),
                'reviewed_by': admin,
                'reviewed_at': timezone.now(),
                'review_logs': [{'step': 1, 'action': 'seeded', 'reviewer': 'system', 'time': timezone.now().strftime('%Y-%m-%d %H:%M:%S')}],
            }
        )
    ...
    for contract_type, rate in [('软件销售', Decimal('0.000300')), ('服务采购', Decimal('0.000500')), ('设备采购', Decimal('0.000300'))]:
        StampTaxRule.objects.get_or_create(
            contract_type=contract_type,
            defaults={'rate': rate, 'description': '默认测试税率', 'is_active': True}
        )

    for name, rule_type in [('付款到期预警', 'payment_due'), ('交付到期预警', 'delivery_due'), ('合同到期预警', 'contract_expiry'), ('发票开具预警', 'invoice_due'), ('目标达成预警', 'target_progress')]:
        AlertRule.objects.get_or_create(
            name=name,
            rule_type=rule_type,
            defaults={'remind_days': 7, 'owner_role': 'operator', 'level': 'medium', 'is_active': True}
        )

    for contract in contracts[:10]:
        AlertMessage.objects.get_or_create(
            contract=contract,
            title=f'到期提醒 - {contract.contract_no}',
            defaults={
                'content': '请关注合同关键节点',
                'warning_type': 'contract_expiry',
                'level': random.choice(['low', 'medium', 'high']),
                'owner': contract.renewal_owner or admin,
    ...
    for owner_value in ['张三', '李四', '销售一部', '华东']:
        target_type = 'salesperson' if owner_value in ['张三', '李四'] else 'department' if owner_value == '销售一部' else 'region'
    ...
    for name, template_type in template_specs:
        DataTemplate.objects.get_or_create(
            ...
            defaults={
                'version': 'v1.0',
                'field_config': [{'field': 'contract_no', 'required': True}, {'field': 'client_name', 'required': True}],
    ...
    DashboardConfig.objects.get_or_create(
        ...
        defaults={
            ...
            'widgets': [
                {'id': 'metric-total', 'type': 'metric', 'title': '累计合同金额', 'metric': 'amount'},
                {'id': 'trend-01', 'type': 'trend', 'title': '月度趋势'},
                {'id': 'rank-01', 'type': 'ranking', 'title': '销售排行', 'field': 'salesperson'},
            ],
            'created_by': admin,
        }
    )
    DashboardConfig.objects.get_or_create(
        ...
        defaults={
            ...
            'widgets': [{'id': 'mobile-metric', 'type': 'metric', 'title': '累计合同金额', 'metric': 'amount'}],
            'created_by': admin,
        }
    )
