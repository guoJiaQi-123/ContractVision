...
from .services import (
    ...
    compute_contract_progress,
    update_contract_metrics,
)


class ContractViewSet(viewsets.ModelViewSet):
    queryset = (
        Contract.objects.select_related("created_by", "renewal_owner")
        .prefetch_related(
            "payment_plans",
            "milestones",
            "change_logs",
    ...
    def _get_approval_process(self, action_type, amount=None):
        queryset = ApprovalProcess.objects.filter(
            action_type=action_type, is_active=True
        ).order_by("-min_amount")
        if amount is None:
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            process = self._get_approval_process(
                ApprovalProcess.ActionType.CREATE, contract.base_amount
            )
            if process:
        ...
        return success_response(
            ...
            message=(
                "合同已创建并提交审批"
                if serializer.instance.approval_status
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        amount = (
            request.data.get("base_amount")
            or request.data.get("amount")
            or instance.base_amount
            or instance.amount
        )
    ...
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        process = self._get_approval_process(
            ApprovalProcess.ActionType.DELETE, instance.base_amount or instance.amount
        )
        if process:
            instance.approval_status = Contract.ApprovalStatus.PENDING
            instance.save(update_fields=["approval_status", "updated_at"])
    ...
    def renewal_summary(self, request):
        ...
        data = {
            "summary": {
                ...
                "renewed": queryset.filter(
                    renewal_status=Contract.RenewalStatus.RENEWED
                ).count(),
                "not_renewed": queryset.filter(
                    renewal_status=Contract.RenewalStatus.NOT_RENEWED
                ).count(),
            },
    ...
    def renew(self, request, pk=None):
        ...
        contract.renewal_reminder_days = request.data.get(
        ...
        return success_response(
            data=ContractSerializer(contract).data, message="续签信息更新成功"
        )
    ...
    def terminate(self, request, pk=None):
        ...
        contract.terminated_at = (
            request.data.get("effective_date") or timezone.now().date()
        )
        ...
        return success_response(
            data=ContractSerializer(contract).data, message="合同状态更新成功"
        )
    ...
    def recalculate_quality(self, request, pk=None):
        ...
        update_contract_metrics(contract)
        return success_response(
            data=ContractSerializer(contract).data, message="合同质量评分已更新"
        )
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if Contract.objects.filter(contract_no=contract_no).exists():
                    error_list.append(
                        {"row": idx + 2, "error": f"合同编号 {contract_no} 已存在"}
                    )
                    continue
                ...
                Contract.objects.create(
                    ...
                    client_contact=(
                        str(row.get("client_contact", "")).strip()
                        if pd.notna(row.get("client_contact"))
                        else ""
                    ),
                    contract_type=(
                        str(row.get("contract_type", "")).strip()
                    ...
                    sign_date=(
                        row.get("sign_date") if pd.notna(row.get("sign_date")) else None
                    ),
                    start_date=(
                        row.get("start_date")
                    ...
                    status=(
                        str(row.get("status", "draft")).strip()
                        if pd.notna(row.get("status"))
                        else "draft"
                    ),
                    salesperson=(
                        str(row.get("salesperson", "")).strip()
    ...
    def export_pdf(self, request):
        ...
        return export_to_pdf(
            serializer.data, columns, title="销售合同数据报表", filename="contracts.pdf"
        )
    ...
    def detail_pdf(self, request, pk=None):
        ...
        payment_status_map = dict(Contract.PaymentStatus.choices)
        delivery_status_map = dict(Contract.DeliveryStatus.choices)
        currency_map = dict(Contract.Currency.choices)
        rows = [
            {"label": "合同编号", "value": data.get("contract_no", "")},
            {"label": "合同标题", "value": data.get("title", "")},
            {"label": "客户名称", "value": data.get("client_name", "")},
            {"label": "客户联系人", "value": data.get("client_contact", "")},
            {"label": "产品类型", "value": data.get("product_type", "")},
            {
                "label": "合同金额",
                "value": f"{data.get('amount', '')} {currency_map.get(data.get('currency', ''), '')}",
            },
            {"label": "签订日期", "value": data.get("sign_date", "")},
            {"label": "开始日期", "value": data.get("start_date", "")},
            {"label": "结束日期", "value": data.get("end_date", "")},
            {
                "label": "合同状态",
                "value": status_map.get(data.get("status", ""), data.get("status", "")),
            },
            {
                "label": "付款状态",
                "value": payment_status_map.get(
                    data.get("payment_status", ""), data.get("payment_status", "")
                ),
            },
            {
                "label": "交付状态",
                "value": delivery_status_map.get(
                    data.get("delivery_status", ""), data.get("delivery_status", "")
                ),
            },
            {"label": "销售人员", "value": data.get("salesperson", "")},
            {"label": "所属部门", "value": data.get("department", "")},
            {"label": "所属区域", "value": data.get("region", "")},
            {"label": "合同描述", "value": data.get("description", "")},
        ]
        columns = [
            {"key": "label", "label": "字段", "pdf_width": 100},
            {"key": "value", "label": "内容", "pdf_width": 380},
        ]
        filename = f"contract_{data.get('contract_no', pk)}.pdf"
        return export_to_pdf(
            rows,
            columns,
            title=f"合同详情 - {data.get('contract_no', '')}",
            filename=filename,
        )

    @action(detail=False, methods=["get"])
    def generate_contract_no(self, request):
        import datetime

        today = datetime.date.today()
        prefix = f'HT{today.strftime("%Y%m%d")}'
        last_contract = (
            Contract.objects.filter(contract_no__startswith=prefix)
        ...
        return success_response(data={"contract_no": contract_no})

    @action(detail=False, methods=["get"])
    def salesperson_options(self, request):
        from apps.users.models import User

        users = (
            User.objects.filter(
                role__in=[User.Role.ADMIN, User.Role.OPERATOR], is_active=True
            )
            .values("id", "username", "role", "department")
            .order_by("username")
        )
